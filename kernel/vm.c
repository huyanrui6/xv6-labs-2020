...
#include "defs.h"
#include "fs.h"
#include "spinlock.h" // must before proc.h
#include "proc.h"

/*
 * the kernel's page table. 全局变量 共享的内核页表
 */
...
// If alloc!=0, create required page-table pages.
//
...
// params: pagetable va pa sz  perm
// uvmmap <= kvmmap
// kvmmap mappages kernel_pagetable
// uvmmap mappjages proc_kernel_pgtbl
// kvmmap对内核页表进行映射，而uvmmap将用于进程的内核页表进行映射
void
...
kvmpa(uint64 va)
{
  ...
  // change global kernel_pagetable to proc kernelpt
...
// params: pagetable, va, npages, do_free
...
// 遍历整个页表。当遇到有效的页表项并且不在最后一层的时候，它会递归调用。
// PTE_V是用来判断页表项是否有效，而(pte & (PTE_R|PTE_W|PTE_X)) == 0 是用来判断是否不在最后一层。
// 因为最后一层页表中页表项中W位，R位，X位起码有一位会被设置为1。
// 注释里面说所有最后一层的页表项已经被释放了，所以遇到不符合的情况就 panic("freewalk: leaf")
void
...
// params: userpt, kernelpt, oldsz, newsz
// 将用户空间的映射添加到每个进程的内核页表，
// 将进程的页表复制一份到进程的内核页表
void
